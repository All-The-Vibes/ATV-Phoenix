...
$taskdir = "$root\tasks"
$cop    = "$env:APPDATA\npm\copilot.cmd"
$results = "$root\results_adv.jsonl"

$TASKS = @(
  @{ name="clamp";    fn="clamp";    spec="clamp_spec.txt";    check="clamp_check.py";    done="def clamp" },
  @{ name="initials"; fn="initials"; spec="initials_spec.txt"; check="initials_check.py"; done="def initials" }
)
