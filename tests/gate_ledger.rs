...
// --- #173: a RED recorded under a different digest is not a vacuous check ---------------------
//
...
// reports saw_red=false. The verdict is right and the message is wrong: it tells the author to go
// reproduce a failure they did reproduce. These two prove the reason tells the digests apart.

#[test]
fn reports_digest_moved_when_red_exists_under_another_digest() {
    let ws = tmp_ws("digestmoved");
    let out = ws.join("out.txt");
    let check = regex_check(&out, "DONE");
    let digest = canonical_digest(&check);
    let tr = Trace::default_in(&ws);
...
fn keeps_the_vacuous_message_when_the_trace_holds_no_red_at_all() {
    ...
    // message was written for and it must keep saying so.
    std::fs::write(&out, "DONE").unwrap();
    tr.append("sense", &digest, true, "regex_in_file", "green").unwrap();
