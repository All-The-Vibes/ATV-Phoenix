...
autonomous run — `/phoenix-goal "<your goal>"`, or plain English like "go", "go autonomous",
...
Phoenix's autonomous entries are the three skills above — `phoenix-goal`, `phoenix-auto` (its autopilot),
and `phoenix-ralph`. There's no bare `/lfg` or `/yolo`; if you type an alias from another harness that maps
to none of them, Phoenix won't silently wing it — it will point you at `phoenix-goal` and confirm the goal +
